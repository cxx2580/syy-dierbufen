  ...
  <div>
    <el-card>
      <div slot="header" class="clearfix">
        <span style="font-size: 18px; font-weight: bold;">销售订单管理</span>
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item label="客户" prop="customerId">
              <el-select v-model="form.customerId" placeholder="请选择客户" style="width: 100%" @change="onCustomerChange">
...
export default {
  ...
  methods: {
    loadOrders() {
      this.$axios.get('/api/sales-order/list', {
    ...
    loadCustomers() {
      this.$axios.get('/api/customer/list', { params: { pageSize: 9999 } })
        .then(res => {
          if (res.data.code === 200) {
    ...
    handleSubmit() {
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          const url = this.isEdit ? '/api/sales-order/update' : '/api/sales-order/add'
    ...
    handleDelete(row) {
      this.$confirm('确定要删除该订单吗？', '提示', {
        ...
        this.$axios.delete(`/api/sales-order/delete/${row.id}`)
          .then(res => {
            if (res.data.code === 200) {
